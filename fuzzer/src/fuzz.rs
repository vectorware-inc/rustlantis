        ...
        match arg.as_str() {
            ...
            "--start" => {
                start = str::parse(
                    &args
                        .next()
                        .ok_or_else(|| "Fuzz start not provided!".to_string())?,
                )
            ...
            "--count" => {
                count = str::parse(
                    &args
                        .next()
                        .ok_or_else(|| "Fuzz count not provided!".to_string())?,
                )
            ...
            "-j" | "--jobs" => {
                threads = str::parse(
                    &args
                        .next()
                        .ok_or_else(|| "Fuzz thread count not provided!".to_string())?,
                )
            ...
            while start.load(Ordering::Relaxed) < end {
                ...
                match test(next, false) {
                    ...
                    Ok(Err(err)) => {
                        ...
                        println!("The CPU and GPU results don't match for {err:?}");
...
fn release_gcc(path: &std::path::Path) -> Result<Vec<u8>, String> {
    ...
    if !output.status.success() {
        return Err(format!(
            "The program at {path:?}, compiled for GPU, exited unsuccessfully:{output:?}"
        ));
    }
    // ... cleanup that executable ...
    std::fs::remove_file(exe_path).map_err(|err| format!("{err:?}"))?;
...
pub type ResultCache = Option<(Vec<u8>, Vec<u8>)>;
