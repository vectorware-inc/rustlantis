...
fn save_reduction(lines: &[String], path: &Path, stage: &str) {
    ...
    for line in lines {
        file.write_all(line.as_bytes())
            .expect("Could not save the reduced example");
    }
}
...
fn test_reduction(lines: &[String], path: &Path, cache: &mut ResultCache) -> bool {
    ...
    let res = crate::fuzz::test_cached(&path, false, cache);
...
fn remove_dump_var(file: &mut Vec<String>, path: &PathBuf) {
    ...
    while curr < file.len() {
        let Some(line) = file[curr..]
            .iter()
            .position(|line| line.contains("dump_var"))
        else {
...
fn match_to_goto(file: &mut Vec<String>, path: &PathBuf, cache: &mut ResultCache) {
    ...
    while curr < file.len() {
        ...
        let Some(match_ends) = file[match_starts..]
            .iter()
        ...
        if test_reduction(&file_copy, path, cache) {
            println!(
                "Reduced {path:?} by {} lines `match_to_goto`",
                match_ends - match_starts
            );
            *file = file_copy;
            curr = match_starts;
        } else {
...
fn block_abort(file: &mut Vec<String>, path: &PathBuf, cache: &mut ResultCache) {
    ...
    while curr < file.len() {
        ...
        if test_reduction(&file_copy, path, cache) {
            println!(
                "Reduced {path:?} by {} lines `block_abort`",
                block_ends - block_starts - 2
            );
            *file = file_copy;
            curr = block_starts;
        } else {
...
fn remove_block(file: &mut Vec<String>, path: &PathBuf, cache: &mut ResultCache) {
    ...
    while curr < file.len() {
        ...
        if test_reduction(&file_copy, path, cache) {
            println!(
                "Reduced {path:?} by {} lines `remove_blocks`",
                block_ends - block_starts
            );
            *file = file_copy;
            curr = block_starts;
        } else {
...
fn remove_fn_calls(file: &mut Vec<String>, path: &PathBuf, cache: &mut ResultCache) {
    ...
    while curr < file.len() {
        let Some(fn_call) = file[curr..]
            .iter()
            .position(|line| line.contains("Call(") && line.contains(" = fn"))
        else {
...
fn remove_fns(file: &mut Vec<String>, path: &PathBuf, cache: &mut ResultCache) {
    ...
    while curr < file.len() {
        ...
        let Some(fn_end) = file[(fn_start + 3)..]
            .iter()
            .position(|line| line.contains("fn fn"))
        else {
            // No more functions to remove - exit early.
            break;
        };
        ...
        if test_reduction(&file_copy, path, cache) {
            println!(
                "Reduced {path:?} by {} lines `remove_fns`",
                fn_end - fn_start
            );
            *file = file_copy;
        } else {
