...
impl TySelect {
    ...
    pub fn choose_ty(&self, rng: &mut impl Rng, tcx: &TyCtxt) -> TyId {
        ...
        if *res.kind(tcx) == TyKind::I128 || *res.kind(tcx) == TyKind::U128 {
            TyCtxt::U64
        } else {
            res
        }
...
fn new_composite(tcx: &mut TyCtxt, rng: &mut impl Rng) {
    let new_ty = match rng.gen_range(0..=3) {
        0 => TyKind::Tuple({
            ...
            (0..length)
                .map(|_| {
                    tcx.indices()
                        .filter(|ty| {
                            *ty != TyCtxt::UNIT && (!(*ty == TyCtxt::U128 || *ty == TyCtxt::I128))
                        })
                        .choose(rng)
                        .unwrap()
                })
        ...
        1 => TyKind::RawPtr(
            tcx.indices()
                .filter(|ty| *ty != TyCtxt::UNIT && (!(*ty == TyCtxt::U128 || *ty == TyCtxt::I128)))
                .choose(rng)
                .unwrap(),
            if rng.gen_bool(0.5) {
                Mutability::Mut
...
fn new_adt(tcx: &mut TyCtxt, rng: &mut impl Rng) {
    ...
    let variants = (0..variant_count).map(|_| {
            ...
            let field_tys = tcx
                .indices()
                 .filter(|ty| *ty != TyCtxt::UNIT && /* https://github.com/rust-lang/rust/issues/119940 */ !ty.contains(tcx, |tcx, ty| ty.is_ref(tcx)) && (!(*ty == TyCtxt::U128 || *ty == TyCtxt::I128)))
